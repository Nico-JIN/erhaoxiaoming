          ...
          <div className="flex items-center justify-between p-4 bg-slate-50 rounded-xl border border-slate-200 hover:border-indigo-300 transition-colors group">
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1">
                <p className="font-medium text-slate-900">{getFilename(resource.file_url)}</p>
                <div className="flex items-center gap-4 text-sm text-slate-500">
                  <span>{resource.file_size || 'Unknown Size'} • {resource.file_type || 'File'}</span>
                  <span className="flex items-center gap-1">
                    <Download size={14} />
                    {formatCount(resource.downloads || 0)}
                  </span>
                </div>
              </div>
            </div>
